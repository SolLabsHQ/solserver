...
describe("PromptPack evidence guidance", () => {
  it("includes evidence claims guidance and budgets in the law section", () => {
    ...
    expect(law).toContain("meta.shape (optional): provide arc/active/parked/decisions/next when possible.");
    expect(law).toContain("meta.affect_signal (optional): single-message affect for the CURRENT user message only.");
    expect(law).toContain("affect_signal.confidence is a bucket: low | med | high.");
    expect(law).toContain("If the user expresses insight");
